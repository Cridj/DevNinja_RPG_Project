...
public class PathFinding : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        isWalking = false;
        rotSpeed = 5f;
        touchSave = playerTM.position;
        touchOrigin = playerTM.position;
        range = 4f;
    ...
    private void FixedUpdate()
    {
        ...
        {
            touchSave = (Vector2)playerTM.position;
        }
    }
    ...
    private void CheckTouch()
    {
        ...
        if (touchSave != touchOrigin)
        {
            ...
            StartFindPath(playerTM.position, touchSave);
        }
    }
    ...
    IEnumerator FindPath(Vector2 startPos, Vector2 targetPos)
    {
        ...
        else
        {
            ...
            Vector3 origin = playerTM.position;
            while (true)
            {
                playerTM.position = Vector2.MoveTowards(playerTM.position, origin, moveSpeed * Time.deltaTime);
                yield return new WaitForSeconds(0.03f);
                if ((int)playerTM.position.x == (int)origin.x && (int)playerTM.position.y == (int)origin.y) break;
            }
        ...
        if (pathSuccess == true)
        {
            ...
            {
                playerTM.position = Vector2.MoveTowards(playerTM.position, wayQueue.First(), moveSpeed * Time.deltaTime);
                if ((Vector2)playerTM.position == wayQueue.First())
                {
                    wayQueue.Dequeue();
                }
